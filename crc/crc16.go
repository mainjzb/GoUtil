...
func CheckCRC16XMODE(buffer []byte, result []byte, order binary.ByteOrder) bool {
	got := GenCRC16XMODEM(buffer)
	want := order.Uint16(result)
	return got == want
}

func CheckCRC16XMODELE(buffer []byte, result []byte) bool {
	got := GenCRC16XMODEM(buffer)
	want := binary.LittleEndian.Uint16(result)
	return got == want
}

func CheckCRC16XMODEBE(buffer []byte, result []byte) bool {
	got := GenCRC16XMODEM(buffer)
